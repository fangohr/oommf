...
<xsl:stylesheet
  ...
  <xsl:template match="/" mode="footer-navigation">
    <xsl:if test="//ltx:navigation/ltx:ref">
      <xsl:choose>
        <xsl:when test="//ltx:navigation/ltx:ref[@rel='prev'] and //ltx:navigation/ltx:ref[@rel='next']">
          <!-- Case: Both prev and next links exist -->
          <xsl:choose>
            <xsl:when test="//ltx:navigation/ltx:ref[@rel='next'] = //ltx:navigation/ltx:ref[@rel='index']">
              <!-- Subcase: next == index; drop index link   -->
              <xsl:text>&#x0A;</xsl:text>
              <xsl:element name="div" namespace="{$html_ns}">
                <xsl:apply-templates select="//ltx:navigation/ltx:ref[@rel='prev']"/>
                <xsl:apply-templates select="//ltx:navigation/ltx:ref[@rel='next']"/>
            ...
            <xsl:otherwise>
              <!-- Subcase: next != index; show prev, index, and next (USUAL CASE) -->
              <xsl:text>&#x0A;</xsl:text>
              <xsl:element name="div" namespace="{$html_ns}">
                <xsl:apply-templates select="//ltx:navigation/ltx:ref[@rel='prev']"/>
                <xsl:apply-templates select="//ltx:navigation/ltx:ref[@rel='next']"/>
        ...
        <xsl:when test="//ltx:navigation/ltx:ref[@rel='prev']">
          <!-- Case: prev exists but next does not; show only prev (we're at index page) -->
          <xsl:text>&#x0A;</xsl:text>
          <xsl:element name="div" namespace="{$html_ns}">
            <xsl:apply-templates select="//ltx:navigation/ltx:ref[@rel='prev']"/>
            <xsl:text>&#x0A;</xsl:text>
        ...
        <xsl:when test="//ltx:navigation/ltx:ref[@rel='next']">
          <!-- Case: next exists but prev does not; show only next (we're at front page) -->
          <xsl:text>&#x0A;</xsl:text>
          <xsl:element name="div" namespace="{$html_ns}">
            <xsl:apply-templates select="//ltx:navigation/ltx:ref[@rel='next']"/>
            <xsl:text>&#x0A;</xsl:text>
        ...
        <!-- No footer if neither prev nor next exist (degenerate case) -->
      </xsl:choose>
    </xsl:if>
  </xsl:template>
