\section{Vector Field Display: mmDisp}\label{sec:mmdisp}%
...
\ssechead{Overview}
The application \app{mmDisp} displays two-dimensional spatial
...
solver.  \app{mmDisp} offers a rich interface for
...
on \htmlref{\app{mmLaunch}}{sec:mmlaunch}, or from the command line via
\begin{verbatim}
...
\item[\optkey{-config file}]
  ...
  \htmlref{detail below}{sec:mmdispconfig}.
...
formats described in Sec.~}{}{sec:vfformats}\index{file!vector~field}.
...
(Sec.~}{)}{sec:vfformats} that \app{mmDisp} recognizes.  When loading
...
(Sec.~}{)}{sec:custom}\index{file!configuration}.  The
...
\htmlref{\app{mmGraph}}{sec:mmgraph} that support ``on-the-fly''
...
\htmlref{configuration files}{sec:mmdispconfig} section below.
...
file are \htmlref{discussed below}{sec:mmdispconfig}.

\ssechead{Controls}\label{sec:mmdispcontrols}
...
\hyperrefhtml{file format}{file format (Sec.~}{)}{sec:vfformats}
...
\end{rawhtml}
\ssechead{Configuration files\label{sec:mmdispconfig}}
...
\item[\optkey{misc,datascale}]
 ...
 \htmlref{\cd{Data Scale} control}{html:mmdispdatascale}.  In general,
...
file \fn{mmdisp.config}.}{pt:mmdisp.config}
\begin{verbatim}
...
\end{rawhtml}
\ssechead{Using \app{mmDisp} as a WWW browser helper application}
...
\hyperrefhtml{Command Line Launching}{Section~}{}{sec:cll}).  On Unix
...
(cf.\ \hyperrefhtml{Command Line Launching}{Section~}{}{sec:cll}).  This will
...
URL:
\begin{center}
\ifnotpdf{\htmladdnormallink{http://math.nist.gov/\~{}MDonahue/cubevortex.ovf}{http://math.nist.gov/\~{}MDonahue/cubevortex.ovf}}
\pdfonly{\htmladdnormallink{http://math.nist.gov/\~{}MDonahue/cubevortex.ovf}{http://math.nist.gov/\%7EMDonahue/cubevortex.ovf}}
\end{center}
