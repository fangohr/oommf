...
\end{verbatim}
  ...
  \begin{itemize}
    \item On the \app{mmLaunch} window, check the \btn{localhost} box,
          ...
          options:
    \begin{itemize}
       \item {\bf{mmArchive:}}\index{application!mmArchive}
                   to auto-save scalar and vector field data
       \item {\bf{mmDataTable:}}\index{application!mmDataTable}
                   to display current values of scalar outputs
       \item {\bf{mmDisp:}}\index{application!mmDisp}
                   to display vector fields
       \item {\bf{mmGraph:}}\index{application!mmGraph}
                   to form x-y plots
       \item {\bf{mmProbEd:}}\index{application!mmProbEd}
                   to grab/modify a problem for \app{mmSolve2D}
                   or \app{Oxsii}
       \item {\bf{mmSolve2D:}}\index{application!mmSolve2D}
                   to control the 2D solver
       \item {\bf{Oxsii:}}\index{application!Oxsii}
                   to control the 3D solver
    \end{itemize}
    \item Click on \btn{mmDisp}, \btn{mmGraph}, and/or
          \btn{mmDataTable}, depending on what form of output you
          want.  Use \btn{mmArchive} to save data to disk.
  \end{itemize}
  \item \quickstartitemlabelsubstart{Run a 2D problem.}
    \begin{description}
    \item[Load problem:]\blankspace
    \begin{itemize}
      \item On the \app{mmLaunch} window, click on the \btn{mmProbEd}
         button.
      \item On the \app{mmProbEd} window, make menu selection
    ...
    \begin{itemize}
      \item On the \app{mmLaunch} window, click on the \btn{mmSolve2D}
            button to launch an instance of the program
            \app{mmSolve2D}\index{application!mmSolve2D}.
      ...
      \item On the \app{mmSolve2D} window:
      \begin{itemize}
    ...
    \begin{itemize}
      \item On the \app{mmSolve2D} window, start the calculation with
      ...
      \item If you requested mmDataTable output, check the boxes for the
            ...
            \app{mmDataTable}\index{application!mmDataTable} window under
      ...
      \item Similarly, check the box for the desired X, Y1, and Y2
            quantities on the \app{mmGraph}\index{application!mmGraph}
            window(s) under the \btn{X}, \btn{Y1} and \btn{Y2} menus.
    \end{itemize}
    \item[Save results:]\index{data!save}\blankspace
    \begin{itemize}
      \item Vector field data (magnetization and effective field) may be
            interactively written to disk\index{file!vector~field} using
            \app{mmDisp}\index{application!mmDisp}, or may be
            automatically saved via scheduled output to
            \app{mmArchive}\index{application!mmArchive}.  For example, to
            save the magnetization state at each control point, start up
            an instance of \app{mmArchive} and select the {\bf
      ...
      \item \btn{DataTable} data\index{file!data~table} may be saved by
            ...
            Alternatively, \app{mmGraph}\index{application!mmGraph}
            can perform this function.  Schedule output to
            \app{mmGraph} as desired, and use either the
            interactive or automated save functionality of
            \hyperrefhtml{\app{mmGraph}}{\app{mmGraph}
            (Sec.~}{)}{sec:mmgraph}.  You can setup the solver data
            scheduling before the calculation is started, but must wait
            for the first data point to configure \app{mmGraph} before
            saving any data.  As a workaround, you may configure
            \app{mmGraph} by sending it the initial solver state
            interactively, and then use the {\btn{Options\pipe clear
            Data}} menu item in \app{mmGraph} to remove the
            initializing data point.
    \end{itemize}
    \item[Midcourse control:]\blankspace
    \begin{itemize}
      \item On the \app{mmSolve2D} window, buttons can stop and restart the
  ...
  \begin{description}
    ...
    \begin{itemize}
      \item On the \app{mmLaunch} window, click on the \btn{Oxsii}
            button to launch an instance of the program
            \app{Oxsii}\index{application!Oxsii}.
      \item Wait for the new solver instance to appear in the 
          \btn{Threads} column in the \app{mmLaunch} window.
    ...
    \item[Load problem:]\blankspace
    \begin{itemize}
      \item On the \app{Oxsii} window, select the
        ...
        \app{mmProbEd} also supports an extension to the \MIF~1.1
    ...
    \item[Start calculation:]\blankspace
    \begin{itemize}
       \item On the \app{Oxsii} window, start the calculation with
    ...
    \begin{itemize}
      \item Vector field data (magnetization and effective field) may be
        ...
        stage, start up an instance of \app{mmArchive} and select
        the {\bf Stage}\index{simulation~3D!stage} check box for
    ...
    \item[Midcourse control:]\blankspace
    \begin{itemize}
       \item On the \app{Oxsii} window, buttons can stop and restart the
