...
command line.
\latex{See the Oxs\_Driver documentation, Sec.~\ref{sec:oxsDrivers}
page~\pageref{html:oxsdrivercheckpoint}, for information on checkpoint
files.}
\html{See the Oxs\_Driver documentation for information on
\htmlref{checkpoint files.}{html:oxsdrivercheckpoint}}
...
\ssechead{Batch Scheduling Systems}
...
\item Atlases
  %begin{latexonly}
   ...
   \hspace*{\tabcolwidth}\=\kill
      Oxs\_BoxAtlas       \> Oxs\_EllipseAtlas \\
      Oxs\_EllipsoidAtlas \> Oxs\_ImageAtlas \\
...
<PRE>
   ...
   <A HREF="#EA">Oxs_EllipsoidAtlas</A>              <A HREF="#IA">Oxs_ImageAtlas</A>
...
\index{Oxs\_Ext~child~classes!Oxs\_EllipseAtlas}%
...
\begin{sloppypar}
\raggedright
\textbf{Examples:} \fn{ellipse.mif},  \fn{ellipsea.mif}.
\end{sloppypar}
...
\begin{sloppypar}
\raggedright
\textbf{Examples:} \fn{ellipsoid.mif} and \fn{ellipsoid.mif}.  See
...
\textbf{Examples:} \fn{imageatlas.mif}, \fn{imageatlas2.mif}, \fn{imagelayers.mif}, \fn{grill.mif}.
...
\fn{pattern.mif}, \fn{tclshapes.mif}, \fn{diskarray.mif},
\fn{ellipsoid-atlasproc.mif}.
\end{sloppypar}
\end{description}
...
\textbf{Examples:} \fn{sample.mif}, \fn{stdprob3.mif}, \fn{stdprob4.mif}.
...
\htmlonlyref{\cd{Oxs\_Demag}}{html:oxsdemag} class supports
...
\textbf{Examples:} \fn{pbcbrick.mif}, \fn{pbcstripes.mif}.
...
\item[Oxs\_Demag:]
   ...
   \begin{latexonly}
      ...
      Specify Oxs\_Demag:\oxsval{name} \ocb\\
        \bi asymptotic\_radius \oxsval{radius}\\
      \ccb
      \end{quote}
   \end{latexonly}
   \begin{rawhtml}<BLOCKQUOTE><DL><DT>
   ...
   offsets larger than \oxsval{radius} (measured in cells) asymptotic
   approximations are used instead.  If \oxslabel{asymptotic\_radius} is
   not specified, then the default value 32 is used.  For non-periodic
   simulations, setting \oxsval{radius} to -1 causes the analytic
   formulae to be used at all offsets.  The example file
   \fn{demagtensor.mif} can be used to extract the computed
   ...
   \textbf{Examples:} \fn{sample.mif}, \fn{cgtest.mif}, \fn{pbcbrick.mif}, \fn{demagtensor.mif}.
...
\begin{quote}
%end{latexonly}
\cd{\begin{tabular}{l@{\hskip 3em}l@{\hskip 3em}l}
    Mx & My & Mz \\
    dMx/dt & dMy/dt & dMz/dt \\
    Total E & dE/dt & pE/pt \\
    \multicolumn{3}{l}{Timestep lower bound}
\end{tabular}}
%begin{latexonly}
\end{quote}
%end{latexonly}
...
\textbf{Examples:} \fn{sample.mif}, \fn{acsample.mif}, \fn{spinmag.mif},
...
\arblink{html:oxsdriverstagecount}
...
The checkpoint\arblink{html:oxsdrivercheckpoint}%
