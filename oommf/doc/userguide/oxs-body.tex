...
tclsh oommf.tcl oxsii [standard options] [-exitondone <0|1>] \
   ...
   [-numanodes nodes] [-parameters params] [-pause <0|1>] \
...
\item[\optkey{-restartfiledir dir}]
  Specifies the directory where restart files are written.  Default is the
  directory holding the \MIF\ file.  You may want to change this to a local
  temporary directory (e.g., \fn{/tmp}), especially if the \MIF\ file
...
field, and vector field outputs as \cd{Oxs\_EulerEvolve}.
