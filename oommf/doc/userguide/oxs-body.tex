...
\htmlonlyref{\cd{Oxs\_ImageAtlas} class}{HTMLoxsImageAtlas}%
\latex{ (Sec.~\ref{sec:oxsAtlases})}, or those using the \MIF~2
\ptlink{\cd{ReadFile}}{PTReadFile} command with the \cd{image}
...
\item[\optkey{-logfile logname}]
  ...
  file is \fn{oommf/oxsii.errors}.\index{file!log}
...
\item[\optkey{-numanodes \boa nodes\bca}]\index{NUMA|(}
  ...
  on \htmlonlyref{threading considerations}{HTMLthreadconsider} in the
  Boxsi documentation.
...
\starssechead{Outputs\label{HTMLoxsiioutputs}}
...
\ptlink{\cd{Schedule}}{PTscheduleCmd} commands in the input
...
\MIF\ \ptlink{Report}{PTMifReport} commands%
...
Error and status messages are written to the log file\index{file!log}
(default \fn{oommf/oxsii.errors}). This is a plain text file where each
record is prefixed with a timestamp and source identification. The start
of each simulation is recorded with a \cd{Start:}\ record, and
an \cd{End} record marks successful completion of the simulation. If you
...
\hyperrefhtml{\app{logreview}}{\app{logreview}
(Sec.~}{)}{sec:logreview}\index{application!logreview} can be used as an
aid for reading \app{Oxsii} log files. See also the command line
application
\hyperrefhtml{\app{lastjob}}{\app{lastjob}
(Sec.~}{)}{sec:lastjob}\index{application!lastjob}, which relies on the
...
\item[\optkey{-kill tags}]
  ...
  \ptlink{\cd{Destination} commands}{PTdestinationCmd}\latex{,
...
\item[\optkey{-logfile logname}]
  Write log and error messages to file \textit{logname}.  The default log
  file is \fn{oommf/boxsi.errors}.\index{file!log}
\item[\optkey{-loglevel level}]
  Controls the detail level of log messages, with larger values of
  \textit{level} producing more output.  Default value is 1.
\item[\optkey{-nice \boa 0\pipe 1\bca}]
...
\item[\optkey{-numanodes \boa nodes\bca}]\index{NUMA|(}
  ...
  on \htmlonlyref{threading considerations}{HTMLthreadconsider} below.
...
\htmlonlyref{\app{Oxsii} documentation}{HTMLoxsiioutputs}\latex{
...
Error and status messages are written to the log file\index{file!log}
(default \fn{oommf/boxsi.errors}). This is in the same log format as
used by \app{Oxsii}. It is a plain text file where each
record is prefixed with a timestamp and source identification. The start
of each simulation is recorded with a \cd{Start:}\ record, and
an \cd{End} record marks successful completion of the simulation. If you
have multiple simulations running simultaneously, either on the same
...
\hyperrefhtml{\app{logreview}}{\app{logreview}
(Sec.~}{)}{sec:logreview}\index{application!logreview} can be used as an
aid for reading \app{Boxsi} log files. See also the command line
application
\hyperrefhtml{\app{lastjob}}{\app{lastjob}
(Sec.~}{)}{sec:lastjob}\index{application!lastjob}, which relies on the
log file to determine the last simulation run and optionally restart
...
\starssechead{Threading considerations\label{HTMLthreadconsider}}
...
\begin{description}
\item[Oxs\_BoxAtlas:]
\pttarget{PTBA}\index{Oxs\_Ext~child~classes!Oxs\_BoxAtlas}%
...
\cd{ColorField} sample proc in the \ptlink{\cd{ReadFile}
documentation}{PTReadFile} \latex{in
...
\pttarget{PTRM}\index{Oxs\_Ext~child~classes!Oxs\_RectangularMesh}%
...
\pttarget{PTPRM}\label{HTMLoxsperiodicrectangularmesh}%
...
\item \textbf{Max Spin Ang:} maximum angle, in degrees, between
  ...
  \cd{Max Spin Ang} is measured as the difference from $180^\circ$,
...
\end{itemize}
   ...
   \ptlink{\cd{GetStateData}}{PTGetStateData} command.
...
\pttarget{PTEP}\index{Oxs\_Ext~child~classes!Oxs\_ExchangePtwise}%
   ...
   model used by \cd{Oxs\_Exchange6Ngbr}.  The exchange
   ...
   thickness model}{HTMLmifvariablethickness} \cite{porter2001}.
...
\end{rawhtml}
   ...
   In addition to the standard energy and field outputs,
   \cd{Oxs\_TwoSurfaceExchange} provides three optional scalar outputs,
   \cd{Max Spin Ang}, \cd{Stage Max Spin Ang}, and
   \cd{Run Max Spin Ang}, as described for
   \ptlink{\cd{Oxs\_Exchange6Ngbr}}{PTE6}. Note,
   however, that ``neighboring spins'' are those that interact through
   exchange coupling. For \cd{Oxs\_TwoSurfaceExchange} this means spins
   ...
   the \oxslabel{report\_max\_spin\_angle} option to 1. These values are
   always accessible inside the \MIF\ file via
   the \ptlink{\cd{GetStateData}}{PTGetStateData} command,
   regardless of the \oxslabel{report\_max\_spin\_angle} setting.
...
\pttarget{PTDE}\index{Oxs\_Ext~child~classes!Oxs\_Demag}%
   ...
   \htmlonlyref{\cd{Oxs\_RectangularMesh}}{HTMLoxsrectangularmesh}
   type; for simulations periodic along one axis direction use the
   \htmlonlyref{\cd{Oxs\_PeriodicRectangularMesh}}{HTMLoxsperiodicrectangularmesh}
...
\pttarget{PTSU}\index{Oxs\_Ext~child~classes!Oxs\_ScriptUZeeman}%
   ...
   \MIF\ \ptlink{\cd{GetStateData}}{PTGetStateData} command;
...
\pttarget{PTTZ}\index{Oxs\_Ext~child~classes!Oxs\_TransformZeeman}%
   A Tcl script specifies a spatially uniform, but time and stage
   varying linear transform that is applied to a spatially varying but
   temporally static field. This essentially combines
   the \cd{Oxs\_ScriptUZeeman} and \cd{Oxs\_FixedZeeman} classes.
...
\end{rawhtml}
   ...
   The \cd{SlidingFieldSpec} proc generates an initialization string
   that is used to create an \cd{Oxs\_ScriptVectorField} vector field
   object, which in turn uses the \cd{SlidingField} proc to specify the
   applied field on a position-by-position basis.  The resulting field
   will be \latex{$2\times 10^4$}\html{2e4} A/m in the positive
   x-direction at all points with relative x-coordinate larger
   than \cd{\$stage/10.}, and 0 otherwise.  \cd{\$stage} is the stage
   index, which here is one of 0, 1, \ldots, 10.  For example,
   if \cd{\$stage} is 5, then the left half of the sample will see
   a \latex{$2\times 10^4$}\html{2e4} A/m field directed to the right,
   and the right half of the sample will see none.  The return value
   from \cd{SlidingFieldSpec} in this case will be
% The extra BLOCKQUOTE's here are a workaround for an apparent
% latex2html bug
\begin{rawhtml}
<BLOCKQUOTE>
...
drivers}{item:MinDriver}.  The drivers hand a magnetization
...
\htmlonlyref{\cd{Oxs\_EulerEvolve}}{HTMLEulerEvolve},
\htmlonlyref{\cd{Oxs\_RungeKuttaEvolve}}{HTMLRungeKuttaEvolve}, and
\htmlonlyref{\cd{Oxs\_SpinXferEvolve}}{HTMLSpinXferEvolve}.
The minimization evolver is
\htmlonlyref{\cd{Oxs\_CGEvolve}}{HTMLCGEvolve}.
\begin{description}
...
The remaining entries, \oxslabel{error\_rate},
...
\htmlonlyref{\cd{Oxs\_EulerEvolve}}{HTMLEulerEvolve} class.
The repeats have the same meaning as in that class, and the same
default values except for \texttt{error\_rate} and
\texttt{relative\_step\_error}, which for \cd{Oxs\_RungeKuttaEvolve}
have default values of 1.0 and 0.01, respectively.  Additionally,
the \oxslabel{alpha}, \oxslabel{gamma\_LL} and \oxslabel{gamma\_G}
...
\htmlonlyref{below}{HTMLoxsrkeprecision}).

The two controls \oxslabel{min\_step\_headroom} and
\oxslabel{max\_step\_headroom} (default values 0.33 and 0.95,
respectively) replace the single \cd{step\_headroom} control option
in the \cd{Oxs\_EulerEvolve} class.  The effective \cd{step\_headroom} is
automatically adjusted by the evolver between the \oxsval{min\_headroom}
and \oxsval{max\_headroom} limits to make the observed reject proportion
approach the \oxslabel{reject\_goal} (default value 0.05).
...
\htmlonlyref{\cd{Oxs\_RungeKuttaEvolve}}{HTMLRungeKuttaEvolve} class:
   \begin{latexonly}
   \begin{quote}\tt
...
The final parameter, \oxslabel{fixed\_spins}, performs the same function
as it does in the \htmlonlyref{\cd{Oxs\_EulerEvolve}}{HTMLEulerEvolve}
class.
...
and
\htmlonlyref{\cd{Oxs\_MinDriver}}{item:MinDriver}
...
Measured in degrees per nanosecond, the \oxslabel{stopping\_dm\_dt}
...
the \oxslabel{stopping\_time} value specifies the maximum ``Simulation
...
\htmlonlyref{\cd{Oxs\_UZeeman}}{HTMLUZeeman}.  The longest of these is
...
See the \ptlink{\cd{SetOptions}}{PTmif2SetOptions}
\latex{\hyperrefhtml{}{(Sec.~}{)}{sec:mif22ExtensionCommands}}
documentation for details.\index{SetOptions~command~(MIF)}
...
the \oxslabel{report\_max\_spin\_angle} option to 1.

The ``Wall time'' output is also disabled by default.  It can be enabled
by setting the \oxslabel{report\_wall\_time} option to 1.  It reports
...
output controls have been moved into the \cd{SetOptions} block.
See the \ptlink{\cd{SetOptions}}{PTmif2SetOptions}
\latex{\hyperrefhtml{}{(Sec.~}{)}{sec:mif22ExtensionCommands}}
documentation for details.\index{SetOptions~command~(MIF)}
...
\oxslabel{report\_wall\_time} option to 1.
...
\oxslabel{cache\_grid}, that takes a mesh specification describing
...
for another \cd{Oxs\_ScriptVectorField} example.
...
varies spatially in a random fashion.  The Specify block has the form:
      ...
      Specify Oxs\_RandomVectorField:\oxsval{name} \ocb\\
       ...
       \bi base\_field \oxsval{vector\_field\_spec}\\
       \bi cache\_grid \oxsval{mesh\_spec}\\
      \ccb
      \end{quote}
      \end{latexonly}
...
There are also two optional parameters, \oxslabel{base\_field} and
\oxslabel{cache\_grid}. If the former is given, then the generated
random field is added onto the field specified by
\cd{vector\_field\_spec}.  If \oxslabel{base\_field} is omitted, the
result is equivalent to specifying a zero field.

The \oxslabel{cache\_grid} option takes a mesh specification that
describes the grid used for spatial discretization caching.  If
\oxslabel{cache\_grid} is not specified, then each call to
\cd{Oxs\_RandomVectorField} generates a different field.  If you want to
...
\oxslabel{cache\_grid} with the appropriate (usually the base problem)
mesh. Note also that if \oxslabel{cache\_grid} is not used, then on
...
\ptlink{random number seed}{PTmif2randomseed}.
\latex{(\cd{RandomSeed}, Sec.~\ref{sec:mif2ExtensionCommands}.)}
...
\pttarget{PTLV}\index{Oxs\_Ext~child~classes!Oxs\_LabelValue}%
...
libraries as needed if they wish to activate other extensions. See
