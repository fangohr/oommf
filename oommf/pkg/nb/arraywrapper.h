...
public:
  ...
  void SetSize(OC_INDEX new_size);  // Throws an exception
  // if size!=0 on entry.  If you want to use this function,
  // call Free() first.

  OC_INDEX GetSize() const { return size; }

  Nb_ArrayWrapper(OC_INDEX initial_size=0) : size(0), objs(NULL) {
...
# if OC_INDEX_WIDTH != 2 && OC_HAS_INT2
  void SetSize(OC_INT2 new_size);
  Nb_ArrayWrapper(OC_INT2 initial_size);
  T& operator[](OC_INT2 i);
  const T& operator[](OC_INT2 i) const;
  void SetSize(OC_UINT2 new_size);
  Nb_ArrayWrapper(OC_UINT2 initial_size);
  T& operator[](OC_UINT2 i);
  const T& operator[](OC_UINT2 i) const;
# endif
# if OC_INDEX_WIDTH != 4 && OC_HAS_INT4
  void SetSize(OC_INT4 new_size);
  Nb_ArrayWrapper(OC_INT4 initial_size);
  T& operator[](OC_INT4 i);
  const T& operator[](OC_INT4 i) const;
  void SetSize(OC_UINT4 new_size);
  Nb_ArrayWrapper(OC_UINT4 initial_size);
  T& operator[](OC_UINT4 i);
  const T& operator[](OC_UINT4 i) const;
# endif
# if OC_INDEX_WIDTH != 8 && OC_HAS_INT8
  void SetSize(OC_INT8 new_size);
  Nb_ArrayWrapper(OC_INT8 initial_size);
  T& operator[](OC_INT8 i);
  const T& operator[](OC_INT8 i) const;
  void SetSize(OC_UINT8 new_size);
  Nb_ArrayWrapper(OC_UINT8 initial_size);
...
{
  if(size==0 && objs==NULL) return; // Nothing to do
  if(size==0 || objs==NULL) {
...
template<class T>
void Nb_ArrayWrapper<T>::SetSize(OC_INDEX new_size)
{
  if(size!=0) {
...
void Nb_2DArrayWrapper<T>::SetSize(OC_INDEX new_size1,OC_INDEX new_size2)
{
  ...
  OC_INDEX totalsize = new_size1*new_size2;

  // Partial overflow check
  if(totalsize<new_size1 || totalsize<new_size2) {
    OC_THROW(Oc_Exception(__FILE__,__LINE__,"Nb_2DArrayWrapper<T>","SetSize",
                NB_ARRAYWRAPPER_ERRBUFSIZE,
                "Integer overflow detected; size1=%lu, size2=%lu",
                static_cast<unsigned long>(new_size1),
                static_cast<unsigned long>(new_size2)));
  }
  size1=new_size1;
  size2=new_size2;
