...
class Oxs_Director;       // Forward declaration
...
struct Oxs_ComputeEnergiesImports;
struct Oxs_ComputeEnergiesExports;
...
struct Oxs_ComputeEnergyData {
  ...
  // Oxs_ComputeEnergiesChunkThread, defined in chunkenergy.cc.  Changes
...
private:
  ...
  friend OC_REAL8m Oxs_ComputeEnergiesErrorEstimate
  (const Oxs_SimState& state,
...
struct Oxs_ComputeEnergiesImports {
public:
  const Oxs_Director& director; // Access to state attach requests
  const Oxs_SimState& state;
  const std::vector<Oxs_Energy*> energies;
  /// Note: This field is usually filled via a call to
  ///   std::vector<Oxs_Energy*> Oxs_Director::GetEnergyObjects() const;
  /// which returns a temporary. In that case, we don't want the
  /// energies member here to be a pointer or reference, lest the
  /// temporary goes away. The compiler may well move the temporary
  /// vector here, and in any case the energies vector shouldn't be very
  /// long.
  ...
  struct AttachRequests {
    OC_BOOL attach_H;
    String attach_H_name;
    // Add additional quantities as needed
    AttachRequests() : attach_H(0) {}
  };

  Oxs_ComputeEnergiesImports
  (const Oxs_Director& idirector,
   const Oxs_SimState& istate,
   const std::vector<Oxs_Energy*>& ienergies,
   const vector<OC_INDEX>* ifixed_spin_list,
   Oxs_MeshValue<OC_REAL8m>* iscratch_energy,
   Oxs_MeshValue<ThreeVector>* iscratch_H)
    : director(idirector),
      state(istate), energies(ienergies),
      fixed_spin_list(ifixed_spin_list),
      scratch_energy(iscratch_energy),
      scratch_H(iscratch_H) {}
};
struct Oxs_ComputeEnergiesExports {
public:
  ...
  OC_REAL8m energy_density_error_estimate; // Estimate of absolute error
  Oxs_Energy::SUMTYPE energy_sum;
  Oxs_Energy::SUMTYPE pE_pt;

  Oxs_ComputeEnergiesExports()
    : energy(nullptr),H(nullptr),
      mxH(nullptr),mxHxm(nullptr),
      max_mxH(0.),energy_density_error_estimate(0.),
      energy_sum(0.),pE_pt(0.) {}
