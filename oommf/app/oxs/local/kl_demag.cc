...
{ // This routine is conceptually const.
  ...
  if(jstop==1) {
	...
	rxcomp[index] -= scale * 
	  ((Oxs_Newell_f(x,0,z-dz)+Oxs_Newell_f(x,0,z+dz))-2*Oxs_Newell_f(x,0,z));
	rxcomp[index] *= 2;
	rycomp[index] -= scale * 
	  ((Oxs_Newell_f(0,x,z-dz)+Oxs_Newell_f(0,x,z+dz))-2*Oxs_Newell_f(0,x,z));
	rycomp[index] *= 2;
	rzcomp[index] -= scale * 
	  ((Oxs_Newell_f(z-dz,0,x)+Oxs_Newell_f(z+dz,0,x))-2*Oxs_Newell_f(z,0,x));
	rzcomp[index] *= 2;
	/// Use f is even in y
      }
    }
  ...
  if(jstop==1) {
	...
	rycomp[index] -= scale * 
	  ((Oxs_Newell_g(x,z-dz,0)+Oxs_Newell_g(x,z+dz,0))-2*Oxs_Newell_g(x,z,0));
	rycomp[index] *= 2;
